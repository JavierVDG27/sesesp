...
<x-app-layout>
    ...
    @php
        ...
        // NUEVO selector rápido
        $proyectosPorEPS = $proyectosPorEPS ?? [];
        $bienesPorProyecto = $bienesPorProyecto ?? [];

        // Áreas ejecutoras (idealmente pásalas desde el controller)
        // Formato recomendado: [ ['clave'=>'01','nombre'=>'...'], ... ]
        $areasEjecutoras = $areasEjecutoras ?? null;
    @endphp
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-lg rounded-2xl p-8">
                ...
                <form action="{{ route('expedientes.store') }}" method="POST" id="form_expediente_create">
                    @csrf
                            ...
                            <div>
                                ...
                                <select name="area_ejecutora" id="area_ejecutora"
                                    ...
                                    @foreach(($instituciones ?? []) as $inst)
                                        <option value="{{ $inst->id }}" @selected((string)old('area_ejecutora')===(string)$inst->id)>
                    ...
                    <div class="mb-6 border border-gray-200 rounded-2xl p-6">
                        ...
                        <input type="hidden" name="capitulo" id="capitulo_hidden" value="{{ old('capitulo') }}">
                        <input type="hidden" name="concepto" id="concepto_hidden" value="{{ old('concepto') }}">
                        <input type="hidden" name="partida_generica" id="pg_hidden" value="{{ old('partida_generica') }}">
                        ...
                        <details class="mt-4" id="modo_avanzado">
                            ...
                            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="border rounded-xl p-4 bg-white">
                                    ...
                                    <ul id="lista_bienes_seleccionados" class="space-y-2 text-sm">
                                        {{-- JS render --}}
                                    </ul>
                    ...
                    <div class="mb-6 border border-gray-200 rounded-2xl p-6">
                        ...
                        <div class="mt-4">
                            ...
                            <input name="tipo_recurso" value="{{ old('tipo_recurso') }}"
                                   class="mt-1 w-full border-gray-300 rounded-md shadow-sm text-sm"
...
window.__fasp = {
  ...
  old: {
    eje: @json(old('eje')),
    programa: @json(old('programa')),
    subprograma: @json(old('subprograma')),
    capitulo: @json(old('capitulo')),
    concepto: @json(old('concepto')),
    partida_generica: @json(old('partida_generica')),
    bienes: @json(old('bienes', [])),
  },
};
</script>
...
(function () {
  ...
  function setEPSFromChip(eje, programa, subprograma) {
    // 1) set eje y reconstruir programas
    ejeSel.value = String(eje);
    onEjeChange(false);

    // 2) set programa y reconstruir subprogramas
    setTimeout(() => {
  ...
  function onEjeChange(keepOld = false) {
    ...
    if (keepOld && old.programa) {
      ...
      // si el usuario cambia eje manualmente, desmarca chip
      clearChipSelection();
    }
  }
  ...
  function onSubprogramaChange(keepOld = false) {
    ...
    // sincronizar chip con selects manuales
    ...
    // restaurar proyecto/bienes si viene de old()
  ...
  if (old.eje) {
    ...
    if (old.programa) {
      // onEjeChange(true) ya intentará restaurar programa/subprograma si hay old.*
    }

    // marcar chip si coincide (y existe)
