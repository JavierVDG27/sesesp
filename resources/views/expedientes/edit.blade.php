...
@php
    ...
    $bloqueado = in_array($estatus, ['en_validacion', 'aprobado']);
    $puedeEnviar = $expediente->puedeEnviarValidacion();
...
<x-app-layout>
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- CABECERA INTERNA (folio + estatus + acciones) --}}
            <div class="bg-white overflow-hidden shadow-lg rounded-2xl p-8 mb-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            ...
                            @if($bloqueado)
                                <span class="text-xs text-gray-500">
                                    Este expediente está bloqueado para edición mientras esté en {{ $badge[1] }}.
                                </span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col items-start md:items-end gap-2">
                        ...
                        {{-- Enviar a validación solo si es borrador o rechazado --}}
                        @if(in_array($estatus, ['borrador','rechazado']))
                            <form action="{{ route('expedientes.enviar-validacion', $expediente) }}" method="POST" class="w-full">
                                @csrf

                                <button type="submit"
                                        @disabled(!$puedeEnviar)
                                        class="w-full md:w-auto inline-flex items-center justify-center px-3 py-2 text-xs font-semibold rounded-md transition
                                            {{ $puedeEnviar ? 'bg-[#9F2241] text-white hover:bg-[#691C32]' : 'bg-gray-200 text-gray-500 cursor-not-allowed' }}">
                                    Enviar a validación →
                                </button>

                                @if(!$puedeEnviar && $estatus === 'rechazado')
                                    <p class="mt-2 text-xs text-red-600">
                                        No puedes reenviar: primero debes realizar cambios después del rechazo.
                                    </p>
                                @endif
                            </form>
                        @endif
                ...
                {{-- Mensajes (SIEMPRE fuera del flex) --}}
                @if (session('success'))
                    <div class="mt-5 px-4 py-3 rounded bg-green-100 text-green-800 text-sm">
                        {{ session('success') }}
                    </div>
                @endif

                @if (session('error'))
                    <div class="mt-5 px-4 py-3 rounded bg-red-100 text-red-800 text-sm">
                        {{ session('error') }}
                    </div>
                @endif
            ...
            {{-- ALERTA DE RECHAZO --}}
        @if($expediente->estaRechazado())
            <div class="mb-6 rounded-xl border border-red-300 bg-red-50 p-5">
                <div class="flex items-start gap-3">
                    <div class="flex-shrink-0 text-red-600 text-xl font-bold">
                        ⚠
                    </div>
                    <div>
                        ...
                        <p class="mt-1 text-sm text-red-700">
                            Este expediente fue rechazado por el validador con la siguiente observación:
                        </p>

                        <div class="mt-3 rounded-md bg-white border border-red-200 p-3 text-sm text-gray-800">
                            {{ $expediente->ultimaObservacionRechazo() ?? 'Sin observaciones registradas.' }}
                        </div>

                        <p class="mt-2 text-xs text-red-600">
                            Corrige la información indicada y vuelve a enviar el expediente a validación.
                        </p>
                    </div>
                </div>
            </div>
        @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="dependencia" id="dependencia"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                                              focus:ring-[#691C32] focus:border-[#691C32]"
                                       value="{{ old('dependencia', $expediente->dependencia) }}"
                                       required
                                       @disabled($bloqueado)>
                            </div>
                            ...
                            <div>
                                ...
                                @php $tipoRecurso = old('tipo_recurso', $expediente->tipo_recurso); @endphp
                            ...
                            <div>
                                ...
                                @php $ejeVal = old('eje', $expediente->eje); @endphp
                                <select name="eje" id="eje"
                                    ...
                                    @foreach (['01','02','03','04','05'] as $eje)
                                        <option value="{{ $eje }}" {{ $ejeVal == $eje ? 'selected' : '' }}>{{ $eje }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                @php $progVal = old('programa', $expediente->programa); @endphp
                                <select name="programa" id="programa"
                                    ...
                                    @foreach (range(1,14) as $num)
                                        ...
                                        <option value="{{ $val }}" {{ $progVal == $val ? 'selected' : '' }}>{{ $val }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                @php $subVal = old('subprograma', $expediente->subprograma); @endphp
                                <select name="subprograma" id="subprograma"
                                    ...
                                    @foreach (range(1,33) as $num)
                                        @php $val = str_pad($num, 2, '0', STR_PAD_LEFT); @endphp
                                        <option value="{{ $val }}" {{ $subVal == $val ? 'selected' : '' }}>{{ $val }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                @php $areaVal = old('area_ejecutora', $expediente->area_ejecutora); @endphp
                            ...
                            <table class="min-w-full text-xs border border-gray-200" id="tabla-presupuesto">
                                <thead class="bg-gray-100 text-gray-700">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 border min-w-[120px]">Partida genérica</th>
                                        <th class="px-3 py-2 border min-w-[320px]">Concepto</th>
                                        <th class="px-3 py-2 border min-w-[180px]">Bien</th>
                                        <th class="px-3 py-2 border min-w-[140px]">Unidad medida</th>
                                        <th class="px-3 py-2 border min-w-[110px]">Cantidad</th>
                                        <th class="px-3 py-2 border min-w-[160px]">Persona</th>
                                        <th class="px-3 py-2 border min-w-[110px]">RLC</th>
                                        <th class="px-3 py-2 border min-w-[160px] text-right">FASP Federal</th>
                                        <th class="px-3 py-2 border min-w-[160px] text-right">FASP Municipal</th>
                                        <th class="px-3 py-2 border min-w-[160px] text-right">FASP Subtotal</th>
                                        <th class="px-3 py-2 border min-w-[160px] text-right">Estatal</th>
                                        <th class="px-3 py-2 border min-w-[160px] text-right">Mun. Estatal</th>
                                        <th class="px-3 py-2 border min-w-[160px] text-right">Est. Subtotal</th>
                                        <th class="px-3 py-2 border min-w-[180px] text-right">Total financ.</th>
                                        <th class="px-2 py-2 border w-12"></th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    @forelse($presupuestos as $i => $p)
                                        <tr class="fila-concepto">
                                            <td class="border px-2 py-1">
                                                <select name="presupuestos[{{ $i }}][capitulo]"
                                                    ...
                                                    @foreach (['1000','2000','3000','4000','5000','6000'] as $cap)
                                                        <option value="{{ $cap }}" {{ ($p['capitulo'] ?? '') == $cap ? 'selected' : '' }}>{{ $cap }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            ...
                                            <td class="border px-2 py-1">
                                                @php $unidadVal = $p['unidad'] ?? ''; @endphp
                                                <select name="presupuestos[{{ $i }}][unidad]"
                                                        class="w-full border-gray-300 rounded-md shadow-sm text-xs"
                                                        @disabled($bloqueado)>
                                                    <option value="">Seleccione...</option>
                                                    @foreach (['Servicio','Lote','Pieza','Póliza','Kit','Licencia','Paquete','Caja','Equipo','Sistema'] as $um)
                                                        <option value="{{ $um }}" {{ $unidadVal == $um ? 'selected' : '' }}>{{ $um }}</option>
                                                    @endforeach
                                                    <option value="otro" {{ $unidadVal == 'otro' ? 'selected' : '' }}>Otro</option>
                                                </select>
                                            </td>
                                            ...
                                            <td class="border px-2 py-1">
                                                ...
                                                <select name="presupuestos[{{ $i }}][rlc]"
                                                    ...
                                                    @foreach (['AE','FC','FC/FM','AEN'] as $op)
                                                        <option value="{{ $op }}" {{ $rlcVal == $op ? 'selected' : '' }}>{{ $op }}</option>
                                                    @endforeach
                                    ...
                                    <tr>
                                        ...
                                        <th class="px-2 py-2 border">
                                            <input type="number" step="0.01"
                                                   class="w-full border-gray-300 rounded-md shadow-sm text-xs text-right"
                        ...
                        @if(!$bloqueado)
                            <button type="submit"
                                    class="inline-flex items-center px-4 py-2 bg-[#691C32] border border-transparent
                                           ...
                                           hover:bg-[#4e1324] focus:outline-none
                                           focus:ring-2 focus:ring-offset-2 focus:ring-[#691C32] transition">
                                Guardar cambios
                            </button>
                        @else
                            <div class="text-sm text-gray-600 flex items-center">
                                No se puede guardar porque el expediente está en <span class="ml-1 font-semibold">{{ $badge[1] }}</span>.
                            </div>
                        @endif
                    </div>
                </form>
            ...
            <div class="mt-6 bg-white overflow-hidden shadow-lg rounded-2xl p-8">
                <h4 class="text-lg font-semibold text-[#691C32] mb-4">Historial y observaciones</h4>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($expediente->historiales as $h)
                                <tr>
                                    ...
                                    <td class="px-4 py-2">{{ optional($h->usuario)->nombres ?? optional($h->usuario)->name ?? 'N/A' }}</td>
                                    <td class="px-4 py-2">
                                        <span class="font-medium">{{ $h->estado_anterior ?? '-' }}</span>
                                        →
                                        <span class="font-medium">{{ $h->estado_nuevo }}</span>
                                    </td>
                                    <td class="px-4 py-2">{{ $h->observaciones ?? '-' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-6 text-center text-gray-500">Sin historial.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        (function() {
            ...
            function getNextIndex() {
                ...
                tabla.querySelectorAll('tbody tr.fila-concepto').forEach(row => {
                    row.querySelectorAll('input[name], select[name]').forEach(el => {
                        ...
                        if (!name) return;
                        const m = name.match(/presupuestos\[(\d+)\]/);
            ...
            function agregarEventosFila(row) {
                ['.input-fasp-federal','.input-fasp-municipal','.input-est-estatal','.input-est-municipal']
                    .forEach(sel => {
                        const input = row.querySelector(sel);
                        if (input) input.addEventListener('input', () => recalcularFila(row));
                    });
            ...
            // Agregar fila (si existe el botón; no existe cuando está bloqueado)
            if (btnAgregar) {
                btnAgregar.addEventListener('click', function() {
                    ...
                    const indice = getNextIndex();

                    // Reindexar nombres
                    nuevaFila.querySelectorAll('input[name], select[name]').forEach(el => {
                        const name = el.getAttribute('name');
                        if (!name) return;
                        el.setAttribute('name', name.replace(/\[\d+\]/, '[' + indice + ']'));
                    });
